...
| 容器名称 | 镜像 | 说明 |
|----------|------|------|
| contract-app | 自建 (docker-app:latest) | 价格附件生成系统应用（uvicorn） |
| contract-nginx | nginx:alpine | 反向代理和静态文件服务 |

### 1.3 部署模式

- **本地开发**: 使用 `docker-compose.yml`，Nginx 监听 80/443 端口
- **远程部署**: 使用 `docker-compose.remote.yml`，Nginx 监听 80/443 端口
...
# 2. 推送镜像到远程服务器（包含 nginx amd64 镜像）
./deploy/docker/scripts/push-image.sh
```

> **注意**: 推送脚本会自动构建 nginx:alpine 的 amd64 版本（适配远程服务器 x86_64 架构）
