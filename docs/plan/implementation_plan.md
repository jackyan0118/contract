...
| 序号 | 问题 | 优先级 | 说明 | 预计修复时间 | 状态 |
|------|------|--------|------|--------------|------|
| 6.11 | 同步调用阻塞事件循环 | P1 | async 函数内部使用同步数据库调用，影响并发性能 | 后续优化 | 待实施 |
| 6.12 | 规则重复加载 | - | 已有缓存实现，无需优化 | - | ✅ 已关闭 |
| 6.13 | 任务访问控制缺失 | P2 | 任务ID未与用户绑定，任何用户可查询任意任务 | 已实施 | ✅ 已完成 |
| 6.14 | 限流器内存泄漏 | - | 已有过期数据清理逻辑，无需优化 | - | ✅ 已关闭 |
| 6.15 | 生产环境 Redis 持久化 | P2 | 内存队列重启后任务丢失，需补充分布式锁 | 后续优化 | 待实施 |
...
| 问题 | 修复说明 |
|------|----------|
| generate.router 未注册 | 在 router.py 中添加了 generate 路由 |
| 认证服务线程安全问题 | 添加了双重检查锁定模式 |
| 时间攻击风险 | 使用 secrets.compare_digest() |
| 开发环境跳过认证 | 改为仅 debug 模式跳过 |
| CORS 配置过于宽松 | 生产环境限制允许来源 |
| 错误信息泄露 | 异常消息不再返回给客户端 |
| 临时文件未清理 | 添加了临时文件清理逻辑 |
